export class Game {
  ...
  async start() {
    try {
      ...
      document.getElementById('game-data').innerHTML = JSON.stringify(this.gameData, null, 4);
    } catch (error) {
      console.log(error);
  ...
  async output(playerResponse) {
    try {
      ...
      const response = await fetch(this.apiBaseUrl + 'api/games/response/' + this.gameToken + '/' + base64EncodedPlayerResponse);
      const data = await response.json();
